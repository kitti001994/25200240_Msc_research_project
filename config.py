...
# Sliding window augmentation switch (training data only).
# Disabled because overlapping windows are near-duplicate per subject and
# encouraged the model to memorise individual subjects instead of the
# autistic/non-autistic signal. With this off, only trial locked windows
# (one per stimulus onset) are used for training.
...
HIDDEN_CHANNELS = [16, 32, 32] # ST-GCN layer output sizes, top layer trimmed from 64 to curb overfitting
TEMPORAL_KERNEL = 3 # temporal conv kernel size
FILM_HIDDEN_DIM = 32 # hidden units inside FiLM MLP
DROPOUT = 0.5 # raised from 0.3, train AUC reached 0.75 while val AUC stayed near 0.5

# Training
BATCH_SIZE = 16
GRAD_ACCUM_STEPS = 2 # micro-batch = BATCH_SIZE // GRAD_ACCUM_STEPS = 8 per forward pass
LR = 2e-4 # lowered from 5e-4, val loss began rising within a few epochs at 5e-4
WEIGHT_DECAY = 5e-3 # L2 reg raised from 1e-3 to slow memorisation of training subjects
EPOCHS = 150
PATIENCE = 30
LABEL_SMOOTHING = 0.1
...
AUGMENT_NOISE_STD = 0.1 # raised from 0.05 to push the model off subject-specific detail

AUGMENT_PARCEL_DROP = 0.1 # raised from 0.02, drops 10% of parcels per window as regularisation
