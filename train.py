"""
Training pipeline with 5-fold cross-validation + Top regions
"""
...
from region_activation_analysis import collect_betas
...
def select_top_regions_for_training_fold(
    fold_idx,
    train_list,
    bids_root,
    atlas_path,
    ...
    """
    Select top Brainnetome parcels using training subjects only.
    """
    os.makedirs(out_dir, exist_ok=True)
    print(
        f"  [Fold {fold_idx+1}] Selecting top regions from "
        f"{len(train_list)} training subjects only."
    )

    beta_df, _, _ = collect_betas(
        subjects=train_list,
        bids_root=bids_root,
        cache_dir=cache_dir,
        atlas_path=atlas_path,
    )
    beta_df.to_csv(os.path.join(out_dir, "fold_train_subject_condition_betas.csv"), index=False)

    effect_stats = compute_effect_stats(beta_df, region_df=None, n_boot=1000)
    effect_stats.to_csv(os.path.join(out_dir, "fold_region_effect_stats.csv"), index=False)

    subject_mats, _, subject_labels = collect_subject_connectivity(
        subjects=train_list,
        masker=masker,
        bids_root=bids_root,
        cache_dir=cache_dir,
    ...
    _, _, node_df = compute_edge_connectivity_stats(
        subject_mats, subject_labels, effect_name="full_run"
    )
    if len(node_df):
        node_df.to_csv(os.path.join(out_dir, "fold_node_graph_summaries.csv"), index=False)

    scored = combine_region_scores(effect_stats, node_df=node_df if len(node_df) else None)
    scored.to_csv(os.path.join(out_dir, "fold_region_scores.csv"), index=False)
    parcel_indices, top_df = select_top_regions(
        scored,
        top_k_per_effect=top_k_per_effect,
        include_contrasts=True,
    )
    top_df.to_csv(os.path.join(out_dir, "fold_top_regions_per_effect.csv"), index=False)

    if not parcel_indices:
        raise RuntimeError("Top-region selection produced zero parcels.")

    print(
        f"  [Fold {fold_idx+1}] Selected {len(parcel_indices)} unique parcels "
        f"from top {top_k_per_effect} per condition/contrast."
    )
    return parcel_indices, top_df
...
def subject_level_auc(probs, labels, subject_ids):
    """
    Aggregate window probabilities to one prediction per subject, then AUC.
    """
    if not subject_ids or len(subject_ids) != len(probs):
        return float("nan")
    subj_probs = defaultdict(list)
    subj_label = {}
    for p, y, sid in zip(probs, labels, subject_ids):
        subj_probs[sid].append(p)
        subj_label[sid] = int(y)
    sids = list(subj_probs.keys())
    if len(sids) < 2:
        return float("nan")
    y_true = [subj_label[s] for s in sids]
    y_prob = [float(np.mean(subj_probs[s])) for s in sids]
    if len(set(y_true)) < 2:
        return float("nan")
    try:
...
def train_one_fold(
    ...
    # dataset
    use_sliding = config.USE_SLIDING_WINDOWS and not debug
    ...
    header = (
        ...
        f"{'vl_loss':>8} {'vl_acc':>7} {'vl_auc':>7} {'sj_auc':>7} {'vl_f1':>7} {'ES':>6}"
    )
    print(header)
    print("-" * len(header))
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        vl_loss, vl_m, vl_probs, vl_labels, _ = run_epoch(
            model, val_loader, optimizer, criterion, device, train=False
        )
        # Subject-level validation AUC drives model selection (see below).
        vl_subj_auc = subject_level_auc(vl_probs, vl_labels, val_ds.subject_ids)
        scheduler.step()
        ...
        sj_auc_str = f"{vl_subj_auc:7.4f}" if not np.isnan(vl_subj_auc) else f"{'nan':>7}"
        print(
            f"{epoch:4d} {tr_loss:8.4f} {tr_m['accuracy']:7.4f} {tr_m['auc']:7.4f} "
            f"{tr_m['f1_macro']:7.4f} {vl_loss:8.4f} {vl_m['accuracy']:7.4f} "
            f"{vl_m['auc']:7.4f} {sj_auc_str} {vl_m['f1_macro']:7.4f} {es_str:>6}"
        )

        selection_auc = vl_subj_auc if not np.isnan(vl_subj_auc) else vl_m["auc"]
        in_warmup = epoch <= warmup_epochs
        if not in_warmup and not np.isnan(selection_auc) and selection_auc > best_val_auc:
            best_val_auc = selection_auc
            patience_counter = 0
            best_model_state = {k: v.cpu().clone() for k, v in model.state_dict().items()}
            print(f"  [Best fold {fold_idx+1}] subj val AUC={selection_auc:.4f}")
        elif not in_warmup:
            patience_counter += 1
...
def write_training_log(mode, hyperparams, dataset_info, results, elapsed_seconds):
    ...
    lines = [
        ...
        f"  Top regions: {hyperparams.get('top_regions', 'disabled')}",
        "",
...
def train(
    ...
    if laughter_mode:
        ...
        print("  Condition regressors zeroed to prevent information leakage.\n")
    ...
    if use_top_regions:
        checkpoint_dir = checkpoint_dir.rstrip("/") + f"_topregions_k{top_k_per_effect}"
        print("\nTOP-REGION MODE: fold-local feature selection enabled.")
        print(
            f"  Top {top_k_per_effect} parcels per condition/contrast are selected "
    ...
    best_history = None
    selected_region_counts = defaultdict(int)
    ...
    for fold_idx, (train_list, val_list) in enumerate(fold_splits):
        ...
        fold_edge_index = edge_index
        fold_edge_weight = edge_weight
        fold_n_parcels = n_parcels
        fold_parcel_indices = None

        if use_top_regions:
            selection_dir = os.path.join(fold_ckpt_dir, "region_selection")
            fold_parcel_indices, fold_top_df = select_top_regions_for_training_fold(
                fold_idx=fold_idx,
            ...
            for pi in fold_parcel_indices:
                selected_region_counts[int(pi)] += 1
            ...
            for subj_id, _ in train_list:
                for run in range(1, 5):
                    ts = get_run_timeseries(subj_id, f"{run:02d}", masker, cache_dir)
                    if ts is not None:
                        fold_ts_list.append(ts[:, fold_parcel_indices])

            graph_cache = os.path.join(
                selection_dir,
                f"population_graph_top{len(fold_parcel_indices)}_t{config.CORR_THRESHOLD}.npz",
            )
            fold_edge_index, fold_edge_weight = build_population_graph(
                fold_ts_list, graph_cache
            )
            fold_edge_index = fold_edge_index.to(device)
            fold_edge_weight = fold_edge_weight.to(device)
            fold_n_parcels = len(fold_parcel_indices)
        ...
        history, best_fold_auc, best_model_state = train_one_fold(
            ...
            edge_weight=fold_edge_weight,
            n_parcels=fold_n_parcels,
            cache_dir=cache_dir,
            ...
            use_film=use_film,
            static_feat_map=static_feat_map,
            parcel_indices=fold_parcel_indices,
        )
        ...
        if best_fold_auc > best_overall_auc and best_model_state is not None:
            ...
            model_tmp = build_selected_model(
                fold_edge_index, fold_edge_weight, n_parcels=fold_n_parcels,
                n_static_features=n_static, use_film=use_film,
            ).to(device)
            ...
            torch.save({
                ...
                "edge_index": fold_edge_index.detach().cpu(),
                "edge_weight": fold_edge_weight.detach().cpu(),
    ...
    if use_top_regions and selected_region_counts:
        freq_df = pd.DataFrame([
            {
                "parcel_index": int(parcel),
                "selected_folds": int(count),
                "selection_frequency": float(count / max(len(fold_results), 1)),
            }
            for parcel, count in selected_region_counts.items()
        ]).sort_values(["selected_folds", "parcel_index"], ascending=[False, True])
        freq_path = os.path.join(checkpoint_dir, "selected_region_frequency.csv")
        freq_df.to_csv(freq_path, index=False)
        print(f"Selected-region frequency saved to {freq_path}")
    ...
    if not os.path.exists(best_model_path):
        if use_top_regions:
            raise FileNotFoundError(
                "best_model.pt not found. Top-region mode cannot create a "
                "fallback model because it needs the fold-specific selected parcels."
            )
    ...
    eval_edge_index = ckpt.get("edge_index", edge_index)
    eval_edge_weight = ckpt.get("edge_weight", edge_weight)
    eval_edge_index = eval_edge_index.to(device)
    eval_edge_weight = eval_edge_weight.to(device)
    eval_n_parcels = int(ckpt.get("n_parcels", n_parcels))
    eval_parcel_indices = ckpt.get("parcel_indices", None)

    # 7. Test dataset (micro-batch)
    micro_bs = max(1, config.BATCH_SIZE // config.GRAD_ACCUM_STEPS)
    test_ds = fMRIWindowDataset(
        test_list, masker, use_sliding=False,
        cache_dir=cache_dir, condition_filter=condition_filter,
        parcel_indices=eval_parcel_indices,
        laughter_mode=laughter_mode,
        static_feat_map=static_feat_map,
    )
    ...
    model = build_selected_model(
        eval_edge_index, eval_edge_weight, n_parcels=eval_n_parcels,
        n_static_features=n_static, use_film=use_film,
    ).to(device)
    ...
    results_full = {
        ...
        "use_top_regions": use_top_regions,
        "top_k_per_effect": top_k_per_effect if use_top_regions else None,
        "selected_parcel_indices": (
            [int(i) for i in eval_parcel_indices]
            if eval_parcel_indices is not None else None
        ),
    }
    ...
    if laughter_mode:
        ...
        mode_str = "full"
    if use_top_regions:
        mode_str += f"_topregions_k{top_k_per_effect}"
    log_results = {
        "best_val_auc": best_overall_auc,
    ...
    write_training_log(
        ...
        hyperparams={
            ...
            "k_folds": n_folds,
            "top_regions": (
                f"top {top_k_per_effect} per condition/contrast"
                if use_top_regions else "disabled"
            ),
        },
...
def train_loso(
    ...
    """
    LOSO cross-validation for subject-level autistic/non-autistic classification.
    """
...
def train_per_condition(
    ...
    for cond_key, cond_name in config.CONDITION_MAP.items():
        ...
        src_dir = checkpoint_dir.rstrip("/") + f"_cond{cond_key}_stgcn_only"
        if use_top_regions:
            src_dir = src_dir + f"_topregions_k{top_k_per_effect}"
        src = os.path.join(src_dir, "best_model.pt")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Outputs to checkpoints_debug/. Use on HPC to verify setup quickly."
        ),
    )
    parser.add_argument(
        "--n-subjects", type=int, default=None, metavar="N",
        help="Limit to N subjects (balanced autistic/non-autistic).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            "Run main training (all conditions), then train one model per condition "
        ),
    )
    parser.add_argument(
        "--per-condition-only", action="store_true",
        help=(
            "Skip main training and run only per-condition models. "
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            "Include static demographic features (age, gender, FSIQ) after "
            "global pooling. Features are z-scored using training-set statistics."
        ),
    )
    parser.add_argument(
        "--top-regions", action="store_true",
        help=(
            "Select top Brainnetome parcels inside each training fold only, "
            "then train/evaluate on those parcels."
        ),
    )
    parser.add_argument(
        "--top-k-per-effect", type=int, default=20, metavar="K",
        help=(
            "Top-region mode"
        ),
    )
    parser.add_argument(
        "--model-suite", action="store_true",
        help=(
            "ST-GCN full, ST-GCN+FiLM full, "
            "ST-GCN+FiLM top regions, ST-GCN+FiLM top regions+demographics, "
            "and ST-GCN top regions without FiLM."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        choices=[1, 2, 3, 4, 5],
        help=(
            "Run LOSO using only windows from condition C (1-5). "
    ...
    if args.model_suite:
        suite = [
            {
                "name": "01_stgcn_full_274",
                "use_film": False,
                "use_top_regions": False,
                "use_demographics": False,
            },
            {
                "name": "02_stgcn_film_full_274",
                "use_film": True,
                "use_top_regions": False,
                "use_demographics": False,
            },
            {
                "name": "03_stgcn_film_top_regions",
                "use_film": True,
                "use_top_regions": True,
                "use_demographics": False,
            },
            {
                "name": "04_stgcn_film_top_regions_demographics",
                "use_film": True,
                "use_top_regions": True,
                "use_demographics": True,
            },
            {
                "name": "05_stgcn_top_regions_no_film",
                "use_film": False,
                "use_top_regions": True,
                "use_demographics": False,
            },
        ]
        for i, spec in enumerate(suite, start=1):
            print("\n" + "=" * 30)
            print(f"MODEL SUITE {i}/{len(suite)}: {spec['name']}")
            print("=" * 30)
            train(
                checkpoint_dir=os.path.join(config.CHECKPOINT_DIR, spec["name"]),
                debug=args.debug,
                n_subjects=args.n_subjects,
                n_epochs=args.n_epochs,
                fresh=args.fresh,
                use_film=spec["use_film"],
                use_demographics=spec["use_demographics"],
                use_top_regions=spec["use_top_regions"],
                top_k_per_effect=args.top_k_per_effect,
            )
    elif args.loso_all_conditions:
        ...
        train(
            ...
            use_film=False,
            use_demographics=use_demo,
            use_top_regions=args.top_regions,
            top_k_per_effect=args.top_k_per_effect,
        )
