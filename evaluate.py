...
def _history_values(history, *keys):
    """Return the first available metric series from a list of possible names."""
    for key in keys:
        if key in history:
            return history[key]
    raise KeyError(f"Missing history keys. Tried: {keys}")
...
def plot_f1(history, fig_dir):
    fig, ax = plt.subplots(figsize=(7, 4))
    epochs = history["epoch"]
    train_f1_asd = _history_values(history, "train_f1_asd", "train_f1_autistic")
    val_f1_asd = _history_values(history, "val_f1_asd", "val_f1_autistic")
    ...
    ax.plot(epochs, train_f1_asd, color=PALETTE["autistic"], lw=1.2, label="Train F1 autistic", linestyle="-", alpha=0.7)
    ax.plot(epochs, val_f1_asd, color=PALETTE["autistic"], lw=1.2, label="Val F1 autistic", linestyle="--", alpha=0.7)
...
def plot_dice(history, fig_dir):
    fig, ax = plt.subplots(figsize=(7, 4))
    epochs = history["epoch"]
    train_dice_asd = _history_values(history, "train_dice_asd", "train_dice_autistic")
    val_dice_asd = _history_values(history, "val_dice_asd", "val_dice_autistic")
    ax.plot(epochs, train_dice_asd, color=PALETTE["train"], lw=1.8, label="Train Dice (autistic)")
    ax.plot(epochs, val_dice_asd, color=PALETTE["val"], lw=1.8, label="Val Dice (autistic)", linestyle="--")
    _annotate_best(ax, epochs, val_dice_asd, "max", PALETTE["val"])
...
def plot_lr_schedule(history, fig_dir):
    ...
    ax.set_yscale("log")
    _epoch_axis(ax, history)
    _save(fig, fig_dir, "13_lr_schedule")
...
def plot_all_metrics_grid(history, fig_dir):
    ...
    panels = [
        ...
        ("train_dice_asd", "val_dice_asd", "Dice (autistic)", "Dice", True, "06"),
    ]

    for idx, (tr_key, vl_key, title, ylabel, higher_better, _) in enumerate(panels):
        row, col = divmod(idx, 3)
        ax = fig.add_subplot(gs[row, col])
        if tr_key not in history and tr_key == "train_dice_asd":
            tr_key = "train_dice_autistic"
        if vl_key not in history and vl_key == "val_dice_asd":
            vl_key = "val_dice_autistic"
...
def plot_descriptive_stats(results_full, fig_dir):
    ...
    metrics_to_plot = [
        ...
        ("bold_iqr", "BOLD IQR"),
        ("bold_skewness", "Skewness"),
        ("bold_kurtosis", "Kurtosis"),
    ]
...
def plot_condition_classification_confusion(results_full, fig_dir):
    """
    Optional 5-condition confusion matrix.

    This is only generated when a condition-classification run writes both
    true and predicted condition labels into results_full.json. The current
    autistic/non-autistic classifier stores test_conds for stratified
    evaluation, but it does not predict condition labels.
    """
    true_keys = ("condition_true", "condition_labels", "test_condition_labels")
    pred_keys = ("condition_pred", "condition_preds", "test_condition_preds")
    y_true = next((results_full.get(k) for k in true_keys if k in results_full), None)
    y_pred = next((results_full.get(k) for k in pred_keys if k in results_full), None)
    ...
    labels = list(range(config.N_CONDITIONS))
    cm = confusion_matrix(y_true, y_pred, labels=labels)
    tick_labels = [
        config.CONDITION_MAP[i + 1].replace("_", "\n")
        for i in labels
    ]
    ...
    ax.set_title("5-Condition Confusion Matrix")
    ...
    fig.tight_layout()
    _save(fig, fig_dir, "15_condition_confusion_matrix")
    return True
...
def plot_loso_roc(loso_results, fig_dir):
    ...
    ax.plot([0, 1], [0, 1], color="#9ca3af", lw=0.8, linestyle=":", label="Chance (0.5)")
    ax.set_xlabel("False Positive Rate (1 - Specificity)")
    ax.set_ylabel("True Positive Rate (Sensitivity)")
    ax.set_title(f"LOSO Subject-Level ROC Curve (N = {n_subj})")
...
def plot_loso_calibration(loso_results, fig_dir):
    ...
    ax.set_ylabel("Fraction autistic (Observed)")
    ax.set_title("LOSO Calibration Curve (Reliability Diagram)")
    ax.set_xlim(-0.02, 1.02)
...
def plot_all_loso(loso_results, fig_dir, fold_histories=None):
    ...
    if sm:
        ...
        print(f"  LOSO summary CSV saved to {csv_path}")
    ...
    print(f"  {n_figs} LOSO figures saved to {fig_dir}/")
...
def plot_all(history, results_full, fig_dir):
    ...
    if plot_condition_classification_confusion(results_full, fig_dir):
        n_figs += 1

    # CSV summary
    save_summary_table(results_full, fig_dir)

    print(f"  {n_figs} figures saved to {fig_dir}/")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate evaluation figures from saved results.")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            "Generate LOSO-specific figures from loso_results.json "
            "instead of the standard K-fold figures."
        ),
    )
    args = parser.parse_args()

    fig_dir = os.path.join(args.checkpoint_dir, "figures")

    if args.loso:
        loso_results_path = os.path.join(args.checkpoint_dir, "loso_results.json")
        if not os.path.exists(loso_results_path):
            raise FileNotFoundError(
                f"loso_results.json not found at {loso_results_path}.\n"
                "Run train.py --loso first."
            )
        with open(loso_results_path, "r") as f:
            loso_results = json.load(f)

        # Load per-fold epoch histories if available (for learning curve)
        fold_hist_path = os.path.join(args.checkpoint_dir, "loso_fold_histories.json")
        fold_histories = None
        if os.path.exists(fold_hist_path):
            with open(fold_hist_path, "r") as f:
                fold_histories = json.load(f)
            print(f"Loaded fold histories for {len(fold_histories)} folds.")
        ...
        results_path = os.path.join(args.checkpoint_dir, "results_full.json")
        ...
        if not os.path.exists(results_path):
            raise FileNotFoundError(
                f"results_full.json not found at {results_path}.\nRun train.py first."
            )
        ...
        plot_all(history=history, results_full=results_full, fig_dir=fig_dir)
